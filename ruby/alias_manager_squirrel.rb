...
#  --swap places
#swap letters of first name and last name
#  --if letter is a,e,i,o,u.. move to the next vowel
#  --if letter is consonant, move to the next consanant

#use string and split into 2 words
#store it to name reverse / switch spots of first
#and last name on index
...
def first_to_last_switch(real_name)
    reverse_name = real_name.split(' ').reverse
    p reverse_name
    spy_scramble(reverse_name)
end

def spy_scramble(reverse_name)
...
outputs = {}
	case input
		...
		when 1
			...
			input = first_to_last_switch(input)
		when 2
